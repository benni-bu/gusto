...
__all__ = ["VerticalHybridizationPC", "ShallowWaterMLPC"]
...
class ShallowWaterMLPC(PCBase):
    """
    A learned preconditioner for the shallow water equations, only for a channel 
    domain for now.
    """

    @timed_function("SWMLInit")
    def initialize(self, pc):
        """
        Set up the problem context.
        ...
        # Extract PC context
        prefix = pc.getOptionsPrefix() + "SW_ML_"
        _, P = pc.getOperators()
        self.ctx = P.getPythonContext()
